...
from chl_forecast.forecasting import train_week1_risk_model


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a 3-level week-1 risk classification model."
    )
    parser.add_argument("--csv", required=True, help="Path to the source CSV file.")
    parser.add_argument(
        "--output-dir",
        default="models_week1_risk",
...
def main() -> None:
    ...
    print(
        f"Saved risk model bundle to {Path(args.output_dir).resolve() / 'week1_risk_model_bundle.joblib'}"
    )
