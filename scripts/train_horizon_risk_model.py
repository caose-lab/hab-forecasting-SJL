...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a horizon-specific 3-level risk model.")
...
def main() -> None:
    args = parse_args()
    report = train_horizon_risk_model(
        csv_path=args.csv,
    ...
    print(
        f"Saved risk model bundle to {Path(args.output_dir).resolve() / f'horizon_{args.horizon}_risk_model_bundle.joblib'}"
    )
